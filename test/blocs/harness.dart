...
class BlocTestHarness {
  ...
  void _buildConnectivity() {
    ...
    when(_connectivity.isOnline).thenReturn(ConnectivityStatus.offline != _connectivity.status);
    when(_connectivity.isOffline).thenReturn(ConnectivityStatus.offline == _connectivity.status);
    when(_connectivity.isWifi).thenReturn(ConnectivityStatus.wifi == _connectivity.status);
    when(_connectivity.isCellular).thenReturn(ConnectivityStatus.cellular == _connectivity.status);

    when(_connectivity.changes).thenAnswer((_) => _connectivity._controller.stream);
    when(_connectivity.whenOffline).thenAnswer((_) => _connectivity._controller.stream.where(
  ...
  void _buildUserBloc() {
    ...
    final UserCredentialsService userService = UserServiceMock.buildAny(
      UserRole.commander,
      _configBloc.repo,
    );
    _userBloc = UserBloc(UserRepository(userService), configBloc);
  }

  void _buildIncidentBloc({UserRole role = UserRole.commander, String passcode = 'T123'}) {
    assert(_withConfigBloc, 'IncidentBloc requires AppConfigBloc');
